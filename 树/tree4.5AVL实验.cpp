...
public:
	...
	{p->dalte = s->dalte;p->data = s->data;
		p->height = s->height;p->left = s->left;p->right = s->right;}
	int swap(bond<T> *p, bond<T> *s);

	int find_destoryed();								//查找不平衡的节点，并将节点地址输入栈中
	bond<T> * find_destoryed_2(bond<T> *pt);
	int find_destoryed_3(bond<T> *pr);
	int AVL();
};
template <typename T>
...
int tree<T>::find_destoryed()
{
	...
	find_destoryed_2(root);
	find_destoryed_3(root);
	return 0;
}
template <typename T>
bond<T> *tree<T>::find_destoryed_2(bond<T> *pt)
{
	if (pt)
	{
		find_destoryed_2(pt->left);
		if (pt)
		{
			if (pt->left || pt->right == NULL);
			else if (pt->left&&pt->right)
			pt->dalte = pt->left->height - pt->right->height;
				
			else if (!pt->left)
				pt->dalte = -pt->right->height;
			
			else if (!pt->right)
				pt->dalte = pt->left->height;
		}
		find_destoryed_2(pt->right);
		if (pt)
		{
			if (pt->left || pt->right == NULL);
			else if (pt->left&&pt->right)
...
template <typename T>
int tree<T>::find_destoryed_3(bond<T> *pr)
{
	if (pr)
	{
		if (abs(pr->dalte) > 1) push_stack(pr);
		if (pr)
		{
			find_destoryed_3(pr->left);
			find_destoryed_3(pr->right);
		}
	}
	return 0;
}
...
int main()
{
	...
	v.find_destoryed();
	for (int i = 0; i <= v.top_tree_stack; i++)
	{
		cout<< v.stack_tree_bond[i] << '\t' << v.stack_tree_bond[i]->dalte << endl;
	}
	v.AVL();
	v.print_tree();
	return 0;
